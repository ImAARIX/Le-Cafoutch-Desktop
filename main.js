...
async function searchForUpdate() {
	...
	var webVersionFile = await axios.get('https://le-cafoutch.com/desktop/version.json');
...
function showUpdateDialog(webVersion) {
	...
	dialog.showMessageBox(null, options).then((response) => {
		if(response.response == "0") {
			shell.openExternal("https://github.com/ImAARIX/le-cafoutch-desktop/releases/tag/v" + webVersion);
			app.quit();
